...
IRsend::IRsend(uint16_t sendpin)
{
...
{
	IRsend *self = (IRsend *) arg;
	ETS_FRC1_INTR_DISABLE();
	digitalWrite(self->sendpin, !digitalRead(self->sendpin));
	ETS_FRC1_INTR_ENABLE();
}
...
void IRsend::enableIROut(int khz)
{
	...
	uint32_t ticks = (CPU_CLK_FREQ / 16 / (khz * 1000) / 2);
...
long IRrecv::decodeSAMSUNG(decode_results *results)
{
	...
	offset++;
	// Check for repeat
	if (irparams.rawlen == 4 && MATCH_SPACE(results->rawbuf[offset], SAMSUNG_RPT_SPACE) && MATCH_MARK(results->rawbuf[offset + 1], SAMSUNG_BIT_MARK))
	{
		results->bits = 0;
		results->value = REPEAT;
